...
| `POST /v1/vaults/{id}/verify` · `POST .../anchor` · `POST .../rotate` | integrity report (a read) · tighten the manifest rollback anchor onto the committed chain head (a **write**: a cached handle never re-opens, so nothing else closes that window) · key rotation (sole-writer contract — 409 for the vault the same process serves over `/mcp`, and since 1.7.0 409 with no class while any other process holds the vault: ROADMAP O257) |
...
- **The open is a read too** (1.0.0, ROADMAP R4 — this bullet used to
  ...
  on `GET /v1/vaults/{id}/stats` alongside `read_only` (where, since 1.7.0,
...
A third property is newer (ROADMAP O216): **an import never replaces a row
awaiting an admission ruling.** A diverted save's 202 names the queue id, and
the data-plane fence scans an import only for the reserved wing's NAME, so a
record declaring an ordinary wing under that id reached the engine and
...
reached the raw writer (§4). The e2e suite (`tests/e2e-orchestrator.sh`, 169 checks,
