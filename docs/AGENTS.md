...
| POST | `/v1/vaults/{id}/backups` | snapshot **this vault**. **409 + `class: "integrity"`** if it fails verification first — never archive a palace that fails its own HMACs, the wire form of the CLI's exit 2. Operator plane |
