...
| GET | `/v1/vaults/{id}/index/status` | remote vector-mirror status (`?backend=`, else `UNDERCROFT_INDEX`): the backend's record count beside the authoritative **local** one, which is the pair a caller diagnosing "is my mirror behind?" needs. A pure read — `index push` is EGRESS and stays absent, which is the boundary this does not share. 502 when the backend is unreachable |
...
| GET | `/v1/vaults/{id}/diary` | one agent's entries (`?agent=` required, `?limit=`). Content-returning; `diary_read` records `ReadOp::Diary` at the store and passes `BulkMember` inward, so the trail says one diary read, not N gets |
...
| GET | `/v1/vaults/{id}/hallways` | entity pairs co-occurring across a wing's drawers (`?wing=` required, `?top=`), as `{a, b, strength}` |
...
| GET | `/v1/vaults/{id}/tunnels/traverse` | wings reachable from `?start=` over tunnels, breadth-first, to `?depth=` (default 2). Returns wing NAMES and depths, never content, so it is **not** a read-audit door |
...
| GET | `/v1/vaults/{id}/tunnels/{tid}/drawers` | recent drawers from the tunnel's destination wing (`?limit=`, default 10). **This returns verbatim content**, so it is a read-audit door — and it needed nothing added, because `follow_tunnel` records `ReadOp::Tunnel` at the STORE, which is where O51 put the witness precisely so a new surface inherits it. Named `/drawers` rather than `/follow` because the path should say what comes back |
