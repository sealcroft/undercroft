...
| `undercroft_index_status` | W | remote vector-mirror record count beside the authoritative local one. **A WRITE**: it calls `ensure`, which CREATES the collection it reports on, so a read-only server refuses it (O83) |
...
| POST | `/v1/vaults/{id}/index/status` | remote vector-mirror status (`?backend=` **required**): the backend's record count beside the authoritative **local** one. **A WRITE, and POST for that reason** — it calls `ensure`, which CREATES the collection (`PUT /collections` on qdrant, `CREATE TABLE` on pgvector), so a `--read-only` server must refuse it and `mutates()` never refuses a GET. It shipped as a GET on the tenant data plane for one day (O68) and is **operator plane** now; the genuinely read-only version needs a per-backend non-creating lookup, filed as **O83**. Because `ensure` runs first, "no mirror" and "empty mirror" both answer `0`. 502 when the backend is unreachable |
