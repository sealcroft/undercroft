...
| **ManifestAnchorHandleRetired** | critical | any `undercroft_anchor_failures_total{class="integrity"}` increase — a post-commit anchor found a `vault.json` the writing handle's keys may not overwrite (another process rotated the vault's keys beside it, or the manifest was edited, removed or written by a newer build), and that handle now refuses every write until the process re-opens the vault. `stats` names the vault under `unhealed` (ROADMAP O254). |
| **ManifestAnchorDeferred** | warning | any `undercroft_anchor_failures_total{class="io"}` increase — a post-commit anchor could not write `vault.json` (a filesystem refusal, or the write lock busy past its timeout). The write is stored and the next anchor covers it. Deliberately NOT on the anchor's lag, `VaultStats.anchor_lag`, which read-audit keeps above zero (ROADMAP O254). |
