...
**143** of the **158** — the rest are prose sections with no `[A-Z][0-9]+` id and
...
e2e checks (a `Content-Length` of 999,999,999,999 over a two-byte body is 413
...
on the orchestrator suite (413 through `/t/`, and `/healthz` after). Tests
...
**The ruling O99 was owed**, put to the maintainer with its measured cost
(~113 of ~595 public items undocumented) and three shapes: enable where
already clean and backlog the rest, enable everywhere now, or record the
by-eye scanner as the method and close as a decision. **Ruled: enable
everywhere now.**

**The measured cost was wrong by 3×, and the ruling was re-confirmed at the
true figure.** `cargo check` with the attribute in place flags **360** items:
170 struct fields and 98 enum variants O99's scanner never counted, plus 92
in its scope (37 methods, 23 structs, 10 enums, 8 constants, 6 associated
functions, 5 modules, 2 traits, 1 associated constant). The scanner counted
top-level and method items; the lint counts every documentable public item.
Reported before a word was written, and the maintainer held the ruling at
360 — after which the TELEMETRY lint found 12 more, the fields of the live
monitor's `Sample`, which exist only behind the `telemetry` feature and are
never compiled by a default check (O84's blind spot, one lint over): **372**
in all. This is the entry's own lesson applied to itself — a figure in a filing
is a claim about the tree — and it is why the number here is the compiler's.

**What shipped.** `#![warn(missing_docs)]` in the root of each of the eight
library crates (core, vault, store, index, llm, net, obs, config), after the
crate doc comment. All 372 items documented against the code they describe:
read in context first, a line where a line is the truth, more where the item
carries a decision. Eight inline struct-variants expanded so each field
carries its own line. Placement verified by the compiler (a doc on a
non-item line does not compile) and the count taken from it: 372 → 0, no
other warning, `cargo clippy --all-targets -- -D warnings` clean on the
default build and both telemetry builds. Two things the unit paid for and
recorded: formatting between the lint run and the edit moved a root file's
lines, so the item table is regenerated by the compiler immediately before
each batch is applied; and an inline `Variant { field: T }` cannot carry a
field doc without being expanded.

**Gate**: the lint itself, under `-D warnings` in the compose `lint` service
and the CI `Lint` job (both, per the `lint parity` preflight). It sees an
item left with NO doc and never a doc attached to the wrong item — the
misattribution half stays with the by-eye scanner O99 recorded as method.
O99's residual paragraph points here.
...
the tree is consistent either way. **RULED 2026-09-07 — enable everywhere,
and the true cost was 360, not 113: O110 (under `## 1.5.1`).**
