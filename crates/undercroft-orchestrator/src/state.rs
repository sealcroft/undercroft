...
#[cfg(test)]
mod rfc3339_tests {
    /// ROADMAP O127: the stamp is RFC 3339, checked at the epoch, at a
    /// leap-day and at a known instant, not by its name.
    #[test]
    fn the_stamp_is_rfc3339() {
        assert_eq!(super::rfc3339_from_unix(0), "1970-01-01T00:00:00Z");
        assert_eq!(
            super::rfc3339_from_unix(951_782_400),
            "2000-02-29T00:00:00Z"
        );
        assert_eq!(
            super::rfc3339_from_unix(1_700_000_000),
            "2023-11-14T22:13:20Z"
        );
        assert!(!super::now_rfc3339().starts_with("unix:"));
    }
}

/// Seconds since the epoch as `YYYY-MM-DDTHH:MM:SSZ`, computed here so the
/// crate stays without a time dependency. `now_rfc3339` used to return
/// `unix:{secs}` (ROADMAP O127): a function so named that wrote a shape no
/// RFC 3339 reader accepts, into `Tenant.created_at`, which `/admin/tenants`
/// serves. Rows written before this carry the old shape; readers display,
/// never parse. Proleptic Gregorian, days-from-civil inverted (Howard
/// Hinnant's algorithm).
